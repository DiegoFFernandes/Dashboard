...
class Pcp extends Model
{
    ...
    public function QtdLotePcp()
    {
        ...
        $key = "qtd_pneus_lote_pcp_" . Auth::user()->id;
    ...
    public function PneusLotePcp()
    {
        $query = ("SELECT
                ...
                CAST(P.NM_PESSOA AS VARCHAR(200) CHARACTER SET UTF8) NM_PESSOA,
                CAST(COALESCE(RC.DS_REGIAOCOMERCIAL,'SEM REGIAO') AS VARCHAR(200) CHARACTER SET UTF8) DS_REGIAOCOMERCIAL,
                SP.DSSERVICO,
        ...
        $key = 'pneus_lote_pcp_' . Auth::user()->id;
        return Cache::remember($key, now()->addMinutes(15), function () use ($query) {
            return DB::connection(Auth::user()->conexao)->select($query);
        });
    }
    public function ItemLotePcp($cd_lote){
        $query = "SELECT IPP.idpedidopneu PEDIDO, OPR.id ORDEM, PP.idpessoa ID, P.NM_PESSOA CLIENTE, SP.dsservico SERVICO,
        ...
        where LOPR.idmontagemlotepcp = $cd_lote
            AND OPR.stexamefinal NOT IN ('A', 'R')
            AND OPR.stordem NOT IN ('F', 'C')
        group by IPP.idpedidopneu, OPR.id, PP.idpessoa, P.NM_PESSOA, SP.dsservico,
        MODELO, MD.dsmedidapneu, PN.nrserie, PN.nrfogo, PN.nrdot, LOPR.idmontagemlotepcp";
